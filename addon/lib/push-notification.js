...
function applyDecorator (target, name, descriptor, options = {}) {
  wrapAndCall (target, 'activate', function () {
    ...
    wrapAndCall (target, 'deactivate', function () {
      ...
      this.messaging.removeNotificationListener (listener);
    });

    // Pass control to the base class, then add the listener.
    this._super.call (this, ...arguments);
    this.messaging.addNotificationListener (listener, { when, preprocess });
  });
}

export default function pushNotification (target, name, descriptor) {
  if (descriptor) {
